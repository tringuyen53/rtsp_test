...
//RTP PACKET
    ...
    let pipeline = gst::parse_launch(&format!(
        ...
        videorate  ! vaapijpegenc ! appsink name=app1 emit-signals=false drop=true sync=false
        thumbnail_video. ! queue leaky=2 ! 
        videorate ! vaapipostproc ! video/x-raw, width=720, height=480 ! vaapijpegenc ! appsink name=app2 emit-signals=false drop=true sync=false" ,
        uri
    ))?
