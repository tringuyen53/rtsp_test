...
fn create_pipeline(uri: String, seed: u8) -> Result<gst::Pipeline, Error> {
    ...
    //    ))?
    let pipeline = gst::parse_launch(&format!(
         "rtspsrc location={} latency=300 ! rtph264depay ! queue leaky=0 ! h264parse ! queue leaky=0 ! vaapih264dec ! videorate ! capsfilter caps=video/x-raw,framerate=3/1 ! queue leaky=0 ! vaapih264enc ! queue leaky=0 ! rtph264pay config-interval=1 ! application/x-rtp, clock-rate=90000, encoding-name=H264, payload=96 ! rtpjitterbuffer latency=300 ! appsink name=sink max-buffers=100 emit-signals=false drop=true" ,
         uri
     ))?
    // let pipeline = gst::parse_launch(&format!(
